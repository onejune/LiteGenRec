#!/usr/bin/env python3
"""
Exp13 V3: HSTU-Lite with Interaction Attention

在 V2 基础上添加 Interaction Attention 层 (参考 V2 Transformer)
"""
...
class HSTULiteCTRV3(nn.Module):
    def __init__(
        ...
        # ⭐ Interaction Attention (参考 V2 Transformer)
...
def main():
    print("=" * 60)
    print("Exp13 V3: HSTU-Lite with Interaction Attention")
    ...
    print("实验结果")
    print("=" * 60)
    print(f"HSTU-Lite V3 AUC: {best_auc:.4f}")
    print(f"HSTU-Lite V2 AUC: 0.7845")
    print(f"V2 基线 (Transformer): 0.7678")
    
    if best_auc > 0.7845:
        print(f"✅ V3 优于 V2 by +{(best_auc - 0.7845) * 100:.2f}bp")
    else:
        print(f"❌ V3 低于 V2 by {(best_auc - 0.7845) * 100:.2f}bp")
    
    torch.save(model.state_dict(), 'hstu_lite_v3_model.pth')
